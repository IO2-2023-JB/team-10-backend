...
namespace Repository.Managers
{
  ...
  internal async Task<string?> CreateIntro((int width, int height) oryginalResolution, string introPath, string id)
  {
      string? location = GetStorageDirectory();
      if (location == null)
          return null;
      ...
      await Cli.Wrap("ffmpeg").WithArguments(new[] { "-i", introPath, "-vf", "scale=" + oryginalResolution.width + 'x' + oryginalResolution.height + ",setsar=1:1", path }).ExecuteBufferedAsync();
