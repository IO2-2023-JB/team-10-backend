...
namespace Repository
{
    ...
    */
    ...
    /*
    await Cli.Wrap("ffmpeg").WithArguments(new[] { "-i", path, "-vf", "setsar=1:1", tempPath }).ExecuteBufferedAsync();

    await Cli.Wrap("ffmpeg").WithArguments(new[] { "-i", introTempPath!, "-i", tempPath, "-filter_complex", "'[0:0][0:1][1:0][1:1]concat=n=2:v=1:a=1[outv][outa]'", "-map", "[outv]", "-map", "[outa]", newPath }).ExecuteBufferedAsync();
    */
